...
| DEC-004 | TMDB is the only active provider in the implemented MVP runtime; IMDb-compatible provider remains deferred behind the provider boundary | Confirmed | Current implementation baseline, user clarification | Keeps current runtime truthful while preserving the planned extension point for a later approved provider | High | Provider ports, normalization, API behavior |
